...
#
# @slim.add_arg_scope
# def __scale(inputs, beta_initializer, gamma_initializer, is_training, data_format,scope=None):
...
#         param_shape= input_shape[-1] if data_format=='NHWC' else input_shape[0:1]
#         # param_shape=(input_shape[1],)
...
def __scale(inputs, beta_initializer, gamma_initializer, is_training, data_format, scope=None):
    ...
    with tf.variable_scope(scope):
        dtype = inputs.dtype.base_dtype
        input_shape = inputs.get_shape()
        param_shape = input_shape[1:2] if data_format == 'NCHW' else input_shape[-1:]
        ...
        if data_format == 'NCHW':
            inputs_rank = input_shape.ndims
            params_shape_broadcast = list(
                [1, input_shape[1].value] + [1 for _ in range(2, inputs_rank)])
...
def __bn_crelu(net):
    ...
    # concat bn and neg-bn
    with tf.name_scope('concat'):
        net = tf.concat([net, neg_net], axis=CONCAT_AXIS)
    net = __scale(net)
    # relu
    net = tf.nn.relu(net, name='relu')
    return net
...
def __inception_block(inputs, block_config):
    ...
    if stride > 1:
        ...
        path_net = __conv_bn_relu(path_net,
                                  ...
                                  scope='poolproj')
        paths.append(path_net)
    block_net = tf.concat(paths, axis=CONCAT_AXIS)
    block_net = __conv(block_net,
                       kernel_size=1,
                       stride=1,
                       num_outputs=inception_outputs,
...
def pvanet(net, include_last_bn_relu=True, freeze_c2=True):
    end_points = {}
    shape2d = tf.shape(net)[2:]
    mult = config.FPN_RESOLUTION_REQUIREMENT * 1.
    new_shape2d = tf.to_int32(tf.ceil(tf.to_float(shape2d) / mult) * mult)
    pad_shape2d = new_shape2d - shape2d
    chan = net.shape[1]
    l = tf.pad(net, tf.stack(
        [[0, 0], [0, 0], [0, pad_shape2d[0]], [0, pad_shape2d[1]]]))
    l.set_shape([None, chan, None, None])
    # conv stage 1
    # /2, for the stride of 2
    conv1_1 = __conv_bn_crelu(l,
                              kernel_size=(7, 7),
                              stride=2,
                              num_outputs=16,
                              scope='conv1_1')
